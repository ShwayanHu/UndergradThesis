...
# 文件路径
file_path = "/Users/yanyan/Documents/MyQuant/MarketSeparationBasedOnNLP/Code（毕业论文）/Data/时间序列向量导出结果"
file_name = "DocVec_monthly_InfoLevel123_feeling(all)_20240329_cutted.csv"
temp_table = read.table(paste(file_path, file_name, sep = "/"), sep=",", header = TRUE)
Y = unname(as.matrix(temp_table[,c(4:13)]))
n=nrow(Y)
...
save_path = "/Users/yanyan/Documents/MyQuant/MarketSeparationBasedOnNLP/Code（毕业论文）/Data/MSVAR检验/检验结果"
save_file_name = paste("States", file_name, sep = '_')
res_states = data.frame(colMeans(states),
                        InfoPublYear = temp_table[c((1+n_AR):nrow(temp_table)),]$InfoPublYear,
                        InfoPublMonth = temp_table[c((1+n_AR):nrow(temp_table)),]$InfoPublMon)
write.csv(res_states, paste(save_path, save_file_name, sep = "/"), row.names = FALSE)